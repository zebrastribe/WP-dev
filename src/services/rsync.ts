...
import {
  ...
  pushRsyncFailureHintAbsolutePath,
} from "../utils/rsync-ssh-env.js";
...
export async function rsyncPull(
  ...
  if (canRetryPullWithRelativePath(remote.path, firstErr)) {
    ...
    throw new Error(`rsync pull failed with exit code ${retry.exitCode}${pullRsyncFailureHint(err)}`);
  }

  throw new Error(`rsync pull failed with exit code ${first.exitCode}${pullRsyncFailureHint(firstErr)}`);
}
...
export async function rsyncPush(
  ...
  const firstErr = first.stderr ?? "";
  if (canRetryPushWithRelativePath(remote.path, firstErr)) {
    const relPath = remote.path.replace(/^\/+/, "");
    const retry = await runPush(relPath);
    if (retry.stderr) process.stderr.write(retry.stderr);
    if (retry.exitCode === 0) {
    ...
    const err = retry.stderr ?? "";
    throw new Error(
      `rsync push failed with exit code ${retry.exitCode}${pushRsyncFailureHint(err, relPath)}`,
    );
  }

  throw new Error(
    `rsync push failed with exit code ${first.exitCode}${pushRsyncFailureHintAbsolutePath(firstErr, remote.path)}`,
  );
}
